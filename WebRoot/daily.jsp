  ...
  <body>
    <div>
    	<s:form action="subDailyAction!execute" namespace="/" method="post" enctype="multipart/form-data">
    		...
    		<hr>
    		<label for="upload">添加图片</label><input type="file" name="upload" accept=".jpg,.gig,.png,.tif,.rgb,.dib,.eps,.jpe,.pcx,.bmp,.gif">
    		<hr>
	...
	<a>动态列表</a>
		...
		<ul>
			...
			<div>
				...
				<tr>
					<td align="center"><a href="deleteDailyAction.action?dailyID=<s:property value="dailyId"/>"><button onclick="return window.confirm('将删除本条动态，你确定要删除吗？')">删除</button></a>
					</td>
				</tr>
			</div>
			</s:iterator>
